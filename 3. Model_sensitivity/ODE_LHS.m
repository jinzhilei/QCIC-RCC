function dydt=ODE_LHS(t,x,LHSmatrix)
%% PARAMETERS %%
Parameter_settings_LHS;

theta_D0_LHS = LHSmatrix(1);  
theta_TN4_LHS = LHSmatrix(2);       
theta_TN8_LHS = LHSmatrix(3);       
theta_M_LHS = LHSmatrix(4); 
kappa_Th_LHS = LHSmatrix(5);         
kappa_Tr_LHS = LHSmatrix(6);         
kappa_Tc_LHS = LHSmatrix(7);            
beta_Th_LHS = LHSmatrix(8);             
beta_Tr_LHS = LHSmatrix(9);          
beta_Tc_LHS = LHSmatrix(10);  
...
X1_D = x(25);   X2_D = x(26);   X3_D = x(27);   X4_D = x(28);
...
proliferation_X1_D = (par.beta_X1 / (1 + par.Lambda_B * (par.B_D^par.n_B / (par.P^par.n_B+par.B_D^par.n_B) ) ) )* (1 - (par.a11 * X1_D + par.a12 * X2_D + par.a13 * X3_D + par.a14 * X4_D)/par.G_X1) * X1_D;
transformation_X2X1 = par.rho_X2X1 * ( par.A_D^par.n_3 / (par.K_A^par.n_3 + par.A_D^par.n_3) ) * X1_D;
transformation_X3X1 = par.rho_X3X1 * ( par.B_D^par.n_3 / (par.K_B^par.n_3 + par.B_D^par.n_3) ) * X1_D;
transformation_X4X1 = par.rho_X4X1 * ( par.A_D^par.n_3 / (par.K_A^par.n_3 + par.A_D^par.n_3) )*( par.B_D^par.n_3 / (par.K_B^par.n_3 + par.B_D^par.n_3) ) * X1_D;
apoptosis_X1_D = par.d_X1 * X1_D;
immune_X1_D = (par. eta_Th * Th_D + par.eta_Tc * Tc_D) * ( 1 + par.Lambda_A * (par.A_D^par.n_A / (par.Q^par.n_A+par.A_D^par.n_A) ) ) * (1/ (Tr_D/par.K_Tr + 1)) * X1_D;

% ====== ARTC ======
proliferation_X2_D = (par.beta_X2 / (1 + par.Lambda_B * (par.B_D^par.n_B / (par.P^par.n_B+par.B_D^par.n_B) ) ) ) * (1 - (par.a21 * X1_D + par.a22 * X2_D + par.a23 * X3_D +par.a24 * X4_D)/par.G_X2) * X2_D;
...
transformation_X4X3 = par.rho_X4X3 * ( par.A_D^par.n_3 / (par.K_A^par.n_3 + par.A_D^par.n_3) ) * X3_D;
apoptosis_X3_D = par.d_X3 * X3_D;
immune_X3_D = (par. eta_Th * Th_D + par.eta_Tc * Tc_D) *  ( 1 + par.Lambda_A * (par.A_D^par.n_A / (par.Q^par.n_A+par.A_D^par.n_A) ) ) * (1/ (Tr_D/par.K_Tr + 1)) * X3_D;
...
differentiation_Th_C = kappa_Th_LHS * (1 + par.lambda_ThI12 * (I12_C / (par.K_I12 + I12_C)) + par.lambda_ThIgamma * (Igamma_C / (par.K_Igamma + Igamma_C))) * (1 / (1 + I10_C / par.K_I10)) * (1 / (1 + Tr_C / par.K_Tr)) * (D_C^par.n_2 / (par.K_D^par.n_2 + D_C^par.n_2))  * TN4_C;
differentiation_Tr_C = kappa_Tr_LHS * (1 + par.lambda_TrTbeta * (Tbeta_C / (par.K_Tbeta + Tbeta_C))) * (1 / (1 + Igamma_C / par.K_TrIgamma)) * (D_C^par.n_2 / (par.K_D^par.n_2 + D_C^par.n_2))  * TN4_C ;
...
proliferation_Th_C = beta_Th_LHS * (1 + par.lambda_I2 * (I2_C / (par.K_I2 + I2_C))) * Th_C ;
...
proliferation_Tr_C = beta_Tr_LHS * Tr_C ;
...
differentiation_Tc_C = kappa_Tc_LHS * (1 + par.lambda_TcI12 * (I12_C / (par.K_I12 + I12_C))) * (1 / (1 + I10_C / par.K_I10)) * (1 / (1 + Tr_C / par.K_Tr)) * (D_C^par.n_2 / (par.K_D^par.n_2 + D_C^par.n_2)) * TN8_C;
...
proliferation_Tc_C = beta_Tc_LHS * (1 + par.lambda_I2 * (I2_C / (par.K_I2 + I2_C))) * Tr_C ;
