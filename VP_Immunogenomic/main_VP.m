...
sample  =  100000;                   % sample: Virtual patients number
pare    =  7;                      % pare: Model heterogeneity parameters number
t_start =  1;                       % t_start: Start time
t_end   =  360;                     % t_end: End time
...
par = [1e6,1e9;
    ...
    0.3,0.55;
    0,0.3;
    0,0.3];% par: Parameter range

%% Parameter Sample

M = LHS(sample,pare,par);  
...
parpool(20);                         % Create a parallel pool
...
parfor i = 1:sample
    ...
    theta_TN4 = M(i,1); theta_TN8 = M(i,2); theta_D0  = M(i,3);theta_M   = M(i,4);
     ...
     beta_Th   = M(i,5); beta_Tr   = M(i,6); beta_Tc   = M(i,7);

    main_parallel(i,t_start,t_end,theta_TN4,theta_TN8,theta_D0,theta_M,beta_Th,beta_Tr,beta_Tc)
