...
function Picture_PK_model_atezolizumab_AUC()
...
P_fill=fill(t_line, drug_line,[144 238 144]./255,'FaceAlpha',0.25,'EdgeColor','none');
hold on

P_mean = plot(1:1:21,mean(AUC_individual,2));
P_mean.LineWidth = 2; 
P_mean.Color = [50 205 50]./255;
...
print('Picture_Atezolizumab_AUC_Cycle1','-dpng','-r600')
...
AUC_function_mean = 1212.6*log(AUC_time)+849.2997;

% cftool(AUC_time,AUC_up)
AUC_function_up = 1645.1*log(AUC_time)+592.4798;

% cftool(AUC_time,AUC_down)
AUC_function_down = 883.4652*log(AUC_time)+998.0330;

P_1 = plot(AUC_time,AUC_function_up,'-','LineWidth',2,'Color',[0 139 0]./255);
hold on
P_2 = plot(AUC_time,AUC_function_mean,'-','LineWidth',2,'Color',[50 205 50]./255);
hold on
P_3 = plot(AUC_time,AUC_function_down,'-','LineWidth',2,'Color',[144 238 144]./255);
hold on

for i=1:21
    P_4 = plot(i,AUC_up(i),'o','Color',[0 139 0]./255);
    hold on
    P_5 = plot(i,AUC_mean(i),'o','Color',[50 205 50]./255);
    hold on
    P_6 = plot(i,AUC_down(i),'o','Color',[144 238 144]./255);
    hold on
end
...
% Set the image name, format, and resolution
set(gcf,'unit','centimeters','position',[10 10 15 12])
print('Picture_Atezolizumab_AUC_Log','-dpng','-r600')
