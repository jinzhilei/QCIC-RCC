function [best_params] = PSO(Par, P_X1,Par_A,dose_A,dim, param_bounds, t_start, t_calibration,pool)
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for i = 1:n_particles
        futures(i) = parfeval(pool, @calculate_error_parallel, 1, positions(i,:), Par,P_X1,Par_A,dose_A,t_start, t_calibration);
    end
    ...
    for iter = 1:n_iterations
        ...
        for i = 1:n_particles
            futures(i) = parfeval(pool, @calculate_error_parallel, 1, ...
                new_positions(i,:), Par, P_X1,Par_A,dose_A,t_start, t_calibration);
        end
    ...
    print('-dpng','-r100','picture\picture_PSO.png');
end


function error = calculate_error_parallel(p, Par,P_X1,Par_A, dose_A,t_start, t_calibration)
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    m = size(Par, 1);
    S = tumor_parameter(m,P_X1,p);
    you = zeros(m, 30);
    pool = gcp('nocreate');
    if isempty(pool)
        for i = 1:m
            y = solve_one_patient(i,Par, S, Par_A,dose_A,t_start, t_calibration,t_calibration);
            you(i,:) = y(end,:);
        end
    else
        futures = parallel.Future.empty();
        for i = 1:m
            futures(i) = parfeval(pool, @solve_one_patient, 1, i, Par, S,Par_A,dose_A, t_start, t_calibration,t_calibration);
        end
    ...
    v = [1 1 1 1];
  data_v = abs([CR-6.06,PR-19.19,SD-36.36,PD-38.38]');
  error = v * data_v;
