...
for p = 1:number_scheme
    ...
    for m = 1:number_drug
        for n = 1:number_drug
            Drug_Ate_init = ratio(p,1) * Drug(1)*Drug_ratio(m);
            Drug_Bev_init = ratio(p,1) * Drug(2)*Drug_ratio(n);
            ...
            S = tumor_parameter(z);
            file_S=strcat('treat_AB/treat',num2str(p),'/(',num2str(m),',',num2str(n),')','/VP_tumor_parameter.mat');
            save(file_S,'S');
            
            B_immune_par= A_immune_par(randi([1 L], z, 1),:);

            C=PK_model_parameter(z);
            file_C=strcat('treat_AB/treat',num2str(p),'/(',num2str(m),',',num2str(n),')','/VP_PK_model_parameter.mat');
            save(file_C,'C');
            ...
            parfor i=1:z
                ...
                treat(a,c,d,i,r,p,m,n,Drug_Ate_init,Drug_Bev_init,ratio,Drug,V_1_A,V_2_A,Q_A,CI_A,V_max_A,K_A_A,V_1_B,V_2_B,Q_B,CI_B,V_max_B,K_B_A,...
