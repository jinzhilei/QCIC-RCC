...
proliferation_X1_D = (par.beta_X1 / (1 + par.Lambda_B * (B_B^par.n_B / (par.P^par.n_B+B_B^par.n_B) ) ) )* (1 - (par.a11 * X1_D + par.a12 * X2_D + par.a13 * X3_D + par.a14 * X4_D)/par.G_X1) * X1_D;
transformation_X2X1 = par.rho_X2X1 * ( A_B^par.n_3 / (par.K_A^par.n_3 + A_B^par.n_3) ) * X1_D;
transformation_X3X1 = par.rho_X3X1 * ( B_B^par.n_3 / (par.K_B^par.n_3 + B_B^par.n_3) ) * X1_D;
transformation_X4X1 = par.rho_X4X1 * ( A_B^par.n_3 / (par.K_A^par.n_3 + A_B^par.n_3) )*( B_B^par.n_3 / (par.K_B^par.n_3 + B_B^par.n_3) ) * X1_D;
apoptosis_X1_D = par.d_X1 * X1_D;
immune_X1_D = (par. eta_Th * Th_D + par.eta_Tc * Tc_D) * ( 1 + par.Lambda_A * (A_B^par.n_A / (par.Q^par.n_A+A_B^par.n_A) ) ) * (1/ (Tr_D/par.K_Tr + 1)) * X1_D;

% ====== ARTC ======
proliferation_X2_D = (par.beta_X2 / (1 + par.Lambda_B * (B_B^par.n_B / (par.P^par.n_B+B_B^par.n_B) ) ) ) * (1 - (par.a21 * X1_D + par.a22 * X2_D + par.a23 * X3_D +par.a24 * X4_D)/par.G_X2) * X2_D;
transformation_X4X2 = par.rho_X4X2 * ( B_B^par.n_4 / (par.K_B^par.n_4 + B_B^par.n_4) ) * X2_D;
apoptosis_X2_D = par.d_X2 * X2_D;
...
transformation_X4X3 = par.rho_X4X3 * ( A_B^par.n_3 / (par.K_A^par.n_3 + A_B^par.n_3) ) * X3_D;
apoptosis_X3_D = par.d_X3 * X3_D;
immune_X3_D = (par. eta_Th * Th_D + par.eta_Tc * Tc_D) *  ( 1 + par.Lambda_A * (A_B^par.n_A / (par.Q^par.n_A+A_B^par.n_A) ) ) * (1/ (Tr_D/par.K_Tr + 1)) * X3_D;
...
differentiation_Th_C = par.kappa_Th * (1 + par.lambda_ThI12 * (I12_C / (par.K_I12 + I12_C)) + par.lambda_ThIgamma * (Igamma_C / (par.K_Igamma + Igamma_C))) * (1 / (1 + I10_C / par.K_I10)) * (1 / (1 + Tr_C / par.K_Tr)) * (D_C^par.n_2 / (par.K_D^par.n_2 + D_C^par.n_2)) * TN4_C;
differentiation_Tr_C = par.kappa_Tr * (1 + par.lambda_TrTbeta * (Tbeta_C / (par.K_Tbeta + Tbeta_C))) * (1 / (1 + Igamma_C / par.K_TrIgamma)) * (D_C^par.n_2 / (par.K_D^par.n_2 + D_C^par.n_2)) * TN4_C ;
...
proliferation_Th_C = par.beta_Th * (1 + par.lambda_I2 * (I2_C / (par.K_I2 + I2_C))) * Th_C ;
...
proliferation_Tr_C = par.beta_Tr * Tr_C ;
...
differentiation_Tc_C = par.kappa_Tc * (1 + par.lambda_TcI12 * (I12_C / (par.K_I12 + I12_C))) * (1 / (1 + I10_C / par.K_I10)) * (1 / (1 + Tr_C / par.K_Tr)) * (D_C^par.n_2 / (par.K_D^par.n_2 + D_C^par.n_2)) * TN8_C;
...
proliferation_Tc_C = par.beta_Tc * (1 + par.lambda_I2 * (I2_C / (par.K_I2 + I2_C))) * Tr_C ;
...
y(29) = ( 1 / par.V_1_A ) * ( par.Q_A * ( A_D - A_B ) - par.CI_A * A_B - par.V_max_A * ( A_B / ( A_B +par. K_A_A ) ) );
y(30) = ( 1 / par.V_2_A ) * ( par.Q_A * ( A_B - A_D ));

%% bevacizumab

y(31) = ( 1 / par.V_1_B ) * ( par.Q_B * ( B_D - B_B ) - par.CI_B * B_B - par.V_max_B * ( B_B / ( B_B + par.K_B_A ) ) );
y(32) = ( 1 / par.V_2_B ) * ( par.Q_B * ( B_B - B_D ));
