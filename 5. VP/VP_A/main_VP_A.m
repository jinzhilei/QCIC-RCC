...
S = tumor_parameter(z);
file_S='./VP_A/VP_tumor_parameter.mat';
save(file_S,'S');
...
file_R='./VP_A/VP_tumor_initialzation.mat';
save(file_R,'r');
...
for i=1:z
    ...
    for j = 1:m
        if j <m
            ...
            yout(t_start:1:t_simend,:) = y(1:nt,:);
        end
    end
...
% S(:,5) = Beta_distribution(0.05,0.15,0.6,0.2,z);     % \beta_X2     %0.1
